...
def sample_dataloader(dataloader, train_size):
    # Get the dataset from the input dataloader
    dataset = dataloader.dataset
    
    # Ensure the requested size doesn't exceed the original dataset
    if train_size > len(dataset):
        raise ValueError(f"Requested training size ({train_size}) exceeds the dataset size ({len(dataset)}).")
    
    # Randomly sample indices from the dataset
    sampled_indices = torch.randperm(len(dataset))[:train_size]
    
    # Create a subset of the dataset with the sampled indices
    subset = Subset(dataset, sampled_indices)
    
    # Create a new DataLoader with the sampled subset
    sampled_dataloader = DataLoader(subset, batch_size=dataloader.batch_size, shuffle=True)
...
def main(param):
    ...
    train_dataloader = sample_dataloader(org_train_dataloader, param)

    print(f"Train dataset size: {len(train_dataloader.dataset)}")

    path = f'/home/ak-research-01/cli/Aligning-CLIP-via-Preference-optimization/models/DS_size_search/IPO/param_{param}'
