...
class DatasetHandler:
    ...
    def ppo_process(self, dataset_name,use_preprocessed_ppo=True): 
        if not use_preprocessed_ppo:
            shuffled_train = []
            for idx in tqdm(self.train_indices):   
                shuffled_train.append((self.data[idx][0], self.data[idx][2],
                                       self.get_random_index(self.NUM_CLASSES, [self.data[idx][2]]),
                                       self.get_random_index(self.NUM_CLASSES, [self.data[idx][2]]),
                                       True))  # (org image, true label, rnd label1, rnd label2, is_original)
                
                shuffled_train.append((self.data[idx][1][0], self.data[idx][2],
                                       self.data[idx][3][0],
                                       self.get_random_index(self.NUM_CLASSES, [self.data[idx][2], self.data[idx][3][0]]),
                                       False))  # (typo image, true label, typo label, rnd label, is_original)
            
            shuffled_test = []   
            for idx in tqdm(self.test_indices):   
                shuffled_test.append((self.data[idx][0], self.data[idx][2],
                                      self.get_random_index(self.NUM_CLASSES, [self.data[idx][2]]),
                                      self.get_random_index(self.NUM_CLASSES, [self.data[idx][2]]),
                                      True))  # (org image, true label, rnd label1, rnd label2, is_original)
                
                shuffled_test.append((self.data[idx][1][0], self.data[idx][2],
                                      self.data[idx][3][0],
                                      self.get_random_index(self.NUM_CLASSES, [self.data[idx][2], self.data[idx][3][0]]),
                                      False))  # (typo image, true label, typo label, rnd label, is_original)
            
            shuffled_val = []   
            for idx in tqdm(self.val_indices):   
                shuffled_val.append((self.data[idx][0], self.data[idx][2],
                                     self.get_random_index(self.NUM_CLASSES, [self.data[idx][2]]),
                                     self.get_random_index(self.NUM_CLASSES, [self.data[idx][2]]),
                                     True))  # (org image, true label, rnd label1, rnd label2, is_original)
                
                shuffled_val.append((self.data[idx][1][0], self.data[idx][2],
                                     self.data[idx][3][0],
                                     self.get_random_index(self.NUM_CLASSES, [self.data[idx][2], self.data[idx][3][0]]),
                                     False))  # (typo image, true label, typo label, rnd label, is_original)
            
            with open(f"/home/ali.rasekh/ambo/final/ppo_preprocessed/{dataset_name}300/shuffled_train_300.p", "wb") as f: 
                pickle.dump(shuffled_train, f)
    
            with open(f"/home/ali.rasekh/ambo/final/ppo_preprocessed/{dataset_name}300/shuffled_test_300.p", "wb") as f: 
                pickle.dump(shuffled_test, f)
            
            with open(f"/home/ali.rasekh/ambo/final/ppo_preprocessed/{dataset_name}300/shuffled_val_300.p", "wb") as f: 
                pickle.dump(shuffled_val, f)
    
            print('Dataset saved!')
        else:
            with open(f"/home/ali.rasekh/ambo/final/ppo_preprocessed/{dataset_name}300/shuffled_train_300.p", "rb") as f: 
                shuffled_train = pickle.load(f)
    
            with open(f"/home/ali.rasekh/ambo/final/ppo_preprocessed/{dataset_name}300/shuffled_test_300.p", "rb") as f: 
                shuffled_test = pickle.load(f)
            
            with open(f"/home/ali.rasekh/ambo/final/ppo_preprocessed/{dataset_name}300/shuffled_val_300.p", "rb") as f: 
                shuffled_val = pickle.load(f)
                    
            print('PPO_processed Dataset is loaded!')

        # Instantiate the dataset
        self.train_subset = CustomDataset(shuffled_train)
        self.test_subset = CustomDataset(shuffled_test)
        self.val_subset = CustomDataset(shuffled_val)
