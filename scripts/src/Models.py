...
class DPO_Model():
    ...
    def trainer(self,num_epochs, train_dataloader,eval_dataloader):
        ...
        for epoch in range(num_epochs):
            ...
            mini_batch_ctr = 0
            for idx,(true_images, typo_images, true_labels, typo_labels) in enumerate(tqdm(train_dataloader)):
...
#             avg_rejected_reward = total_rejected_reward / len(train_dataloader)
    ...
    def test(self,test_dataloader,prompts):
        ...
        with torch.no_grad():
            for idx,(true_images, typo_images, true_labels, typo_labels) in enumerate(tqdm(test_dataloader)):
...
Accuracy On Typographic Dataset:\n
    ...
    def validation(self,val_dataloader,epoch,model):
        ...
        batch_size = b = val_dataloader.batch_size
        iters_per_epoch = total_samples // batch_size

        with torch.no_grad():
            for idx,(true_images, typo_images, true_labels, typo_labels) in enumerate(tqdm(val_dataloader)):
                ...
                probs, logprobs, logits_per_item = model(texts, images)
